...
const menuGroups = [
  ...
  },
  {
    ...
    icon: Activity,
    children: [
      { href: '/admin/leads', label: 'Leads Management', badge: 12, badgeTone: 'amber' },
      { href: '/admin/bookings', label: 'Booking Requests', badge: 8, badgeTone: 'emerald' },
  ...
  },
  {
    ...
    children: [
      ...
      { href: '/admin/settings?tab=whatsapp', label: 'WhatsApp Settings' },
...
export default function AdminLayout({ title, children }) {
  ...
  const [openGroup, setOpenGroup] = useState(() => localStorage.getItem('admin_sidebar_open') || 'dashboard')
  const { showWarning, countdownSeconds, stayLoggedIn, logoutNow } = useAdminAutoLogout()
  ...
  const filteredGroups = useMemo(() => {
    ...
    return menuGroups
      .map((group) => ({
        ...group,
        children: group.children.filter((item) => item.label.toLowerCase().includes(term)),
      }))
      .filter((group) => group.label.toLowerCase().includes(term) || group.children.length > 0)
  }, [search])
  ...
  const alertConfig = useMemo(() => {
    if (props?.flash?.error) return { type: 'error', message: props.flash.error }
    if (firstValidationError) return { type: 'error', message: firstValidationError }
    if (props?.flash?.success) return { type: 'success', message: props.flash.success }
    return null
  ...
  return (
    <div className={darkMode ? 'dark h-screen overflow-hidden bg-slate-950 text-slate-100' : 'h-screen overflow-hidden bg-slate-100 text-slate-700'}>
      <div className="flex h-full">
        <aside
          className={`${
            collapsed ? 'md:w-[88px]' : 'md:w-[280px]'
          } relative hidden md:block h-screen shrink-0 overflow-hidden border-r border-slate-200 bg-white p-4 shadow-[0_10px_40px_rgba(15,23,42,0.08)] backdrop-blur-xl transition-all duration-200 dark:border-slate-800 dark:bg-slate-950`}
        >
            ...
            <div className="rounded-2xl border border-slate-200 bg-slate-50 p-3 shadow-sm dark:border-slate-800 dark:bg-slate-900/70">
              <div className="flex items-start justify-between gap-2">
                <div className="min-w-0 flex-1">
                  <div className="grid h-12 w-full place-items-center rounded-xl bg-slate-950 ring-1 ring-slate-700 shadow-sm">
                    <img
                  ...
                  {!collapsed && (
                    <div className="mt-2 min-w-0">
                      <p className="truncate text-sm font-semibold leading-5 text-slate-900 dark:text-slate-100">UNO Trips</p>
                ...
                <button
                  ...
                  className="rounded-lg border border-slate-200 p-1.5 text-slate-500 hover:bg-slate-100 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-800"
            ...
            <div className="mt-4 flex-1 overflow-y-auto pr-1 [scrollbar-color:#cbd5e1_transparent] [scrollbar-width:thin]">
              <nav className="space-y-2">
                {filteredGroups.map((group) => {
                  ...
                  const activeChildIndex = group.href ? -1 : group.children.findIndex((item) => isItemActive(url, item.href))
                  const groupActive = group.href ? isItemActive(url, group.href) : activeChildIndex !== -1
                  const isOpen = openGroup === group.key
                  if (group.href) {
                    return (
                      <div key={group.key} className="rounded-2xl border border-slate-200 bg-white p-1 dark:border-slate-800 dark:bg-slate-900/80">
                        <Link
                          href={group.href}
                          className={`group flex w-full items-center justify-between rounded-xl px-3 py-2.5 text-sm transition ${
                            groupActive
                              ? 'bg-gradient-to-r from-indigo-500/15 to-blue-500/15 text-indigo-700 dark:text-indigo-300'
                  ...
                  return (
                    <div key={group.key} className="rounded-2xl border border-slate-200 bg-white p-1 dark:border-slate-800 dark:bg-slate-900/80">
                      ...
                      >
                        ...
                        {!collapsed && <ChevronDown className={`h-4 w-4 transition-transform ${isOpen ? 'rotate-180' : 'rotate-0'}`} />}
                      </button>
                      {!collapsed && (
                        <div className={`grid overflow-hidden transition-all duration-200 ${isOpen ? 'grid-rows-[1fr] opacity-100' : 'grid-rows-[0fr] opacity-0'}`}>
                          <div className="min-h-0 space-y-1 pl-3 pr-2 pb-2">
                            {group.children.map((item, idx) => {
                              const active = idx === activeChildIndex
                              return (
                                <Link
                                  ...
                                  className={`group/item relative flex items-center justify-between rounded-lg px-3 py-2 text-[13px] transition ${
                                    active
                                      ? 'mt-3 bg-gradient-to-r from-indigo-600 to-blue-600 text-white shadow-md'
                                      : 'text-slate-600 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-800'
                                  }`}
                                >
                                  <span className="truncate">{item.label}</span>
                                  {item.badge ? (
                                    <span className={`rounded-full px-2 py-0.5 text-[10px] font-semibold ${badgeStyles[item.badgeTone] || 'bg-slate-100 text-slate-600'}`}>
                                      {item.badge}
                                    </span>
                                  ) : null}
                                  {active ? <span className="absolute left-0 top-1/2 h-5 w-1 -translate-y-1/2 rounded-r-full bg-white/90" /> : null}
                                </Link>
                              )
                            })}
                          </div>
        ...
        <div className="flex min-w-0 flex-1 flex-col overflow-hidden">
          <header className="sticky top-0 z-10 flex h-16 shrink-0 items-center justify-between border-b border-slate-200 bg-white/95 px-5 backdrop-blur dark:border-slate-800 dark:bg-slate-950/90">
            <div>
              <h1 className="text-lg font-semibold text-slate-800 dark:text-slate-100">{title}</h1>
            </div>
            <div className="flex items-center gap-2">
              <button className="grid h-9 w-9 place-items-center rounded-xl border border-slate-200 bg-white text-slate-500 hover:bg-slate-100 dark:border-slate-700 dark:bg-slate-900">🔔</button>
              <button
                onClick={refreshCurrentPage}
                disabled={refreshing}
                className="inline-flex items-center gap-2 rounded-lg border border-slate-200 px-3 py-1.5 text-xs text-slate-700 hover:bg-slate-100 disabled:opacity-60 dark:border-slate-700 dark:text-slate-100 dark:hover:bg-slate-800"
              >
                <RefreshCw className={`h-3.5 w-3.5 ${refreshing ? 'animate-spin' : ''}`} />
                Refresh
              </button>
              <button onClick={() => setDarkMode((v) => !v)} className="rounded-lg border border-slate-200 px-2.5 py-1.5 text-xs dark:border-slate-700">
                {darkMode ? 'Light' : 'Dark'}
              </button>
              <button onClick={logoutNow} className="rounded-lg bg-gradient-to-r from-indigo-600 to-blue-600 px-3 py-1.5 text-sm font-medium text-white hover:from-indigo-500 hover:to-blue-500">Logout</button>
            </div>
          </header>
          {alertConfig && dismissedAlert !== alertConfig.message ? (
            ...
            >
              ...
              <button
                onClick={() => setDismissedAlert(alertConfig.message)}
          ...
          <footer className="sticky bottom-0 z-10 shrink-0 border-t border-slate-200 bg-white/95 px-6 py-4 text-sm text-slate-500 backdrop-blur dark:border-slate-800 dark:bg-slate-950/95 dark:text-slate-400">© 2026, Made with ❤️ by UNO Trips Admin</footer>
        </div>
      </div>
      {showWarning ? (
        <div className="fixed inset-0 z-[120] flex items-center justify-center bg-stone-950/50 p-4 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-2xl border border-amber-200 bg-white p-5 shadow-2xl dark:border-stone-700 dark:bg-stone-900">
            <h3 className="text-lg font-semibold text-stone-900 dark:text-amber-50">Session Expiring</h3>
